...
export function buildMockJournals() {
    const journals = [];
    for (let week = 1; week <= mockEnrollment.totalWeeks; week++) {
        ...
        if (week <= 3) {
            journals.push({
                ...
                submittedDate: `${startDate.slice(0, 4)}-${startDate.slice(5, 7)}-${String(Number(startDate.slice(8, 10)) + 5).padStart(2, "0")}`,
                hasFeedback: week <= 2,
            });
        }
        else if (week === 4) {
...
export const mockFeedbacks = {
    1: {
        ...
        date: "2026-03-09",
        content: "첫 주차 적응 잘 하고 있는 것 같습니다. 업무 내용을 조금 더 구체적으로 작성하면 좋겠습니다.",
    },
    2: {
        journalId: 2,
        supervisorName: "김지은 교수",
...
export const mockAdminStats = {
    ...
    completionRate: Math.round(mockAdminStudents.reduce((sum, s) => sum + s.completionRate, 0) / mockAdminStudents.length),
    formTemplateCount: 1,
...
export function buildMockSupervisorJournals() {
    ...
    names.forEach((name, idx) => {
        for (let week = 1; week <= 3; week++) {
            ...
            journals.push({
                ...
                feedbackContent: status === "REVIEWED" ? `${name} 학생, ${week}주차 일지 잘 작성했습니다. 계속 수고해주세요.` : undefined,
