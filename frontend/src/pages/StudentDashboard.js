...
import { CalendarDays, CheckCircle2, ChevronLeft, ChevronRight, Circle, MessageSquare, PencilLine, X, } from "lucide-react";
const statusLabel = {
    WRITING: "미작성",
    SUBMITTED: "작성완료",
    REVIEWED: "검토완료",
...
export default function StudentDashboard() {
    ...
    const [writeWeek, setWriteWeek] = React.useState(null);
    ...
    const draftCount = journals.filter((j) => j.status === "WRITING" && Object.values(j.content).some((v) => v && v.trim().length > 0)).length;
    ...
    function openWrite(journal) {
        setWriteWeek(journal.week);
        setDraft({ ...journal.content });
        setDraftDates({ startDate: journal.startDate ?? "", endDate: journal.endDate ?? "" });
    }
    function closeWrite() {
        setWriteWeek(null);
    }
    ...
    async function saveJournal(submit) {
        if (!currentJournal)
            return;
        setSaving(true);
        const payload = {
            ...
            status: (submit ? "SUBMITTED" : "WRITING"),
        };
        try {
        ...
        setJournals((prev) => prev.map((j) => j.id === currentJournal.id
            ? {
                ...j,
                ...payload,
    ...
    return (<div className="min-h-screen bg-slate-50">
        ...
        {recentFeedbacks.length > 0 && (<Card>
            ...
            <CardContent className="space-y-3">
              {recentFeedbacks.map((j) => {
                ...
                return (<button key={j.id} onClick={() => openWrite(j)} className="w-full text-left p-3 rounded-md border border-slate-200 hover:border-blue-300 hover:bg-blue-50 transition-colors">
          ...
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {journals.map((j) => (<tr key={j.id} className="border-b border-slate-100 last:border-0">
      ...
      {/* 일지 작성/조회 풀스크린 팝업 */}
      {writeWeek !== null && currentJournal && enrollment && (<div className="fixed inset-0 z-50 bg-white flex flex-col">
          {/* 헤더 */}
          <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4 flex-shrink-0">
            ...
            <div className="flex items-center gap-3">
              <Button size="icon" variant="ghost" disabled={currentIdx <= 0} onClick={() => navigateWeek(-1)} aria-label="이전 주차">
                <ChevronLeft className="w-5 h-5"/>
              </Button>
              <h2 className="text-lg font-bold whitespace-nowrap">{currentJournal.week}주차 일지</h2>
              <Button size="icon" variant="ghost" disabled={currentIdx < 0 || currentIdx >= sortedJournals.length - 1} onClick={() => navigateWeek(1)} aria-label="다음 주차">
          ...
          <div className="flex-1 flex overflow-hidden">
            <div className="flex-1 flex flex-col gap-4 p-6 overflow-y-auto">
              {isEditable ? (<div className="grid grid-cols-2 gap-4 max-w-md">
                  <div className="space-y-1.5">
                    <Label>시작일</Label>
                    <Input type="date" value={draftDates.startDate} onChange={(e) => setDraftDates((d) => ({ ...d, startDate: e.target.value }))}/>
                  </div>
                  <div className="space-y-1.5">
                    <Label>종료일</Label>
                    <Input type="date" value={draftDates.endDate} onChange={(e) => setDraftDates((d) => ({ ...d, endDate: e.target.value }))}/>
                  </div>
                </div>) : (<p className="text-sm text-slate-500">
                  실습 기간: {currentJournal.startDate ?? "-"} ~ {currentJournal.endDate ?? "-"}
                </p>)}
              ...
              {!isEditable && (<p className="text-xs text-slate-400">
            ...
            <div className="w-80 flex-shrink-0 border-l border-slate-200 p-6 overflow-y-auto space-y-3">
              ...
              {feedbackData ? (<div className="space-y-1">
                ...
                </div>) : (<p className="text-sm text-slate-400">등록된 피드백이 없습니다.</p>)}
            </div>
          </div>
