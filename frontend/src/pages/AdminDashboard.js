...
import { BarChart3, ClipboardList, Download, FileText, Plus, Users } from "lucide-react";
export default function AdminDashboard() {
    const [stats, setStats] = React.useState(mockAdminStats);
    const [students, setStudents] = React.useState(mockAdminStudents);
    const [templates, setTemplates] = React.useState(mockFormTemplates);
    ...
    const [exportSemester, setExportSemester] = React.useState("1");
    ...
    return (<div className="min-h-screen bg-slate-50">
              ...
              <CardContent>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {students.map((s) => (<tr key={s.id} className="border-b border-slate-100 last:border-0">
                          ...
                          <td className="py-3 pr-4">
                            <div className="flex items-center gap-2">
                              <div className="w-32 h-2 bg-slate-100 rounded-full overflow-hidden">
                                <div className="h-full bg-blue-500 rounded-full" style={{ width: `${s.completionRate}%` }}/>
                              </div>
            ...
            {templates.map((t) => (<Card key={t.id}>
                ...
                <CardContent>
                  <ul className="text-sm text-slate-600 space-y-1">
                    {t.fields.map((f) => (<li key={f.key} className="flex items-center gap-2">
            ...
            <Card>
              ...
              <CardContent className="space-y-4 max-w-md">
                ...
                <label className="flex items-center gap-2 text-sm">
                  <input type="checkbox" checked={includeFeedback} onChange={(e) => setIncludeFeedback(e.target.checked)} className="w-4 h-4 rounded border-input"/>
                  피드백 포함하여 다운로드
                </label>
      ...
      <NewTemplateDialog open={newTemplateOpen} onClose={() => setNewTemplateOpen(false)} onCreate={handleAddTemplate} nextId={templates.length + 1}/>
    </div>);
}
function NewTemplateDialog({ open, onClose, onCreate, nextId, }) {
    ...
    return (<Dialog open={open} onClose={onClose} title="새 양식 추가" footer={<>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-2">
            {fields.map((field, idx) => (<div key={idx} className="grid grid-cols-[1fr_1fr_110px_36px] gap-2 items-center">
                <Input placeholder="key (예: tasks)" value={field.key} onChange={(e) => updateField(idx, { key: e.target.value })}/>
                <Input placeholder="항목명 (예: 주요 수행 업무)" value={field.label} onChange={(e) => updateField(idx, { label: e.target.value })}/>
